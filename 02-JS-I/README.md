...
```javascript
    ...
    let apellido = 'Perez';
    const comidafavorita = 'Pizza'; // ¿Debería ser const?
```
...
`Math` también tiene métodos que redondearán los números para nosotros. `.round` redondeará un número al número entero más cercano. `.floor` siempre redondeará un número al número entero menor más cercano. `.ceil` siempre se redondeará al número entero mayor más cercano.
